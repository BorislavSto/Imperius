            ...
            steps{
                script{
                    withEnv(["UNITY_PATH=${UNITY_INSTALLATION}"]){
                        // Run EditMode tests
                        bat'''
                        "%UNITY_PATH%/Unity.exe" -projectPath "%WORKSPACE%" -runTests -testPlatform editmode -logFile "%WORKSPACE%/test-results-editmode.log" -testResults editmodetests.xml
                        '''
                        
                        // Run PlayMode tests
                        bat'''
                        "%UNITY_PATH%/Unity.exe" -projectPath "%WORKSPACE%" -runTests -testPlatform playmode -logFile "%WORKSPACE%/test-results-playmode.log" -testResults playmodetests.xml
                        '''
                    }
                }
            }
